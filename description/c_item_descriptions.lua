...
function bindID()
  bindKey ( "lalt", "down", showNearbyItemDescriptions )
  bindKey ( "lalt", "up", RemoveID )
  bindKey ( "ralt", "down", toggleNearbyItemDescriptions )
end
addEventHandler ( "onClientResourceStart", resourceRoot, bindID )

function toggleNearbyItemDescriptions()
	if enabled then
		RemoveID()
	else
		showNearbyItemDescriptions()
	end
end

function RemoveID( key, keyState )
	if enabled then
		enabled = false
        items = {}
		removeEventHandler ( "onClientRender", getRootElement(), showItemNote )
	end
end

local function wrapItem(itemValue)
	local text = { "" }
...
function showNearbyItemDescriptions()
	...
	if not enabled then
		...
		for index, nearbyObject in ipairs( exports.global:getNearbyElements(getLocalPlayer(), "object") ) do
			if isElement(nearbyObject) then
                if not (getElementData(nearbyObject, "itemID")==72) then
                    if getElementData(nearbyObject, "itemID") then
                        local itemID = getElementData(nearbyObject, "itemID")
                        local itemName = exports.global:getItemName(itemID)

                        items[nearbyObject] = wrapItem(itemName)
                    end
                end
			end
		end
		addEventHandler("onClientRender", getRootElement(), showItemNote)
	end
end

function showItemNote()
	for theObject, note_text in pairs(items) do
