				...
				if px and isLineOfSightClear(cx, cy, cz, entrance.x, entrance.y, entrance.z, true, false, false, true, true, false, false) then	
					...
					if powner < 1 and pfaction < 1 and ptype ~= 2 then -- IF FOR SALE
						...
						if ptype == 3 then -- RENTABLE
							toBeAdded = "K pronájmu: $"..exports.global:formatMoney(pcost).."\n"
							toBeShowed = toBeShowed..toBeAdded
							local len = dxGetTextWidth(toBeAdded)
							if len > fontWidth then
								fontWidth = len
							...
							toBeAdded = "K prodeji: $"..exports.global:formatMoney(pcost).."\n"
							toBeShowed = toBeShowed..toBeAdded
							local len = dxGetTextWidth(toBeAdded)
							if len > fontWidth then
								fontWidth = len
				    ...
				    if exports.interior_system:canPlayerSeeInteriorID(theProperty) then
						toBeAdded = "Číslo popisné: "..getElementData(theProperty,"dbid").."\n"
						toBeShowed = toBeShowed..toBeAdded
						local len = dxGetTextWidth(toBeAdded)
						if len > fontWidth then
							fontWidth = len
					...
					--START DRAWING
					local marg = 5
					local oneLineHeight = dxGetFontHeight(1, "default")
					...
					dxDrawText(toBeShowed, px, py, px + fontWidth, (py + fontHeight), textColor, 1, "default", "center")
				end
			end
		end
	end
